...
class modpso:
    ...
    def initialize_particles(self, node_preferences, num_information, node_to_community, total_nodes,
                             total_communities):
        """
        使用基于度的初始化方法初始化粒子的位置和速度
        """
        nodes = list(self.graph.nodes)
        degree_sorted_nodes = sorted(nodes, key=lambda n: self.graph.degree[n], reverse=True)

        for _ in range(self.num_particles):
            # 基于度选择前 k 个节点作为初始解
            particle = degree_sorted_nodes[:self.budget]

            # 扰动操作：随机替换部分节点
            for idx in range(len(particle)):
                if random.random() > 0.5:  # 50% 概率替换
                    available_nodes = list(set(nodes) - set(particle))
                    if available_nodes:
                        particle[idx] = random.choice(available_nodes)
